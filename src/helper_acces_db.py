...
def get_nexus(anio=2020,cache=False):  
    ...
    if anio!=2020 :
        df = pd.DataFrame(columns = ["COD_MOD","ratio_C","ratio_N","ratio_O"])
        return df
        
    filename = 'BD_Docentes_NEXUS'
    extension = '.dta'
    df_nexus_group = None
    ...
    url_nexus = url_template.format(filename,extension)
    url_cache = url_template.format(filename+"_cache",extension)
    
    df_nexus = pd.read_stata(url_nexus,columns=["codmod",'sitlab',"codniveduc","estplaza"])   

    df_nexus = df_nexus[(df_nexus.codniveduc.isin(["A1","A2","A3","A5","B0","F0"]) == True) &
                        (df_nexus.codmod.isin(["0000000","0000001","0000002","-041418","-598060"]) == False) &
                        (df_nexus.sitlab!="B") & (df_nexus.estplaza!="BLOQ")
                       ]
